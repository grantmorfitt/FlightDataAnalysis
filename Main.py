...
Var = {"G97S_DSP_YTSIMTM_F4_1_","G04_EOM_ALT_AGL_F8_1_", "G04_EOM_CAS_F8_1_", "G34_NAV_GS_DEVDDM_F4_1_","L34_NAV_LOC_DEVDDM_F4_1_"}
#time, altitude, CAS
...
#vref,airspeed,gsDeviation, locDeviation
for currentSample in range(len(time)): #This calculates stability using IsStable function
    stability[currentSample] = IsStable(75,velocity[currentSample],gsDev[currentSample],locDev[currentSample])
...
df = pd.concat([height,velocity,gsDev,locDev,df],axis = 1) #; df = pd.concat(velocity) ;  df = pd.concat(gsDev) ; df = pd.concat(locDev)
